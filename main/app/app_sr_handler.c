...
void sr_handler_task(void *pvParam)
{
    ...
    while (true) {
        ...
        app_mqtt_get_client(&mqtt_client);
        ...
        if (ESP_MN_STATE_DETECTED & result.state) {
            ...
            case SR_CMD_LIGHT_ON:
                msg_id = esp_mqtt_client_publish(mqtt_client, "lighting_state", "true", 0, 1, 0);
                ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
                app_pwm_led_set_power(1);
                break;
            case SR_CMD_LIGHT_OFF:
                msg_id = esp_mqtt_client_publish(mqtt_client, "lighting_state", "false", 0, 1, 0);
                ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
            ...
            case SR_CMD_AC_ON:
                msg_id = esp_mqtt_client_publish(mqtt_client, "fcu_znsp", "70", 0, 1, 0);
                ui_sensor_set_ac_poweron();
                break;

            case SR_CMD_AC_OFF:
                msg_id = esp_mqtt_client_publish(mqtt_client, "fcu_znsp", "65", 0, 1, 0);
